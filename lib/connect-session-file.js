    ...
    fs.readdir( self.path, function(err, files){
        ...
        files.forEach(function(d) {
            ...
            if( !fs.existsSync(filePath) ) {
                util.warn('reap '+filePath+' does not exist');
                return;
            }
...
FileSessionStore.prototype.get = function(sid, fn) {
    ...
    var dirName = crypto.createHash('md5').update(sid).digest('hex');
    ...
    fs.exists( filePath, function (exists) {
        if( exists ){
            fs.readFile( filePath, function (err, data) {
                if(err || data.length <= 0){
                    if( printDebug ) { util.warn(serial + ' get FAIL [' + filePath + '] - no data found'); }
                    fn();
                }else{
...
FileSessionStore.prototype.set = function (sid, sess, fn) {
    var serial = this.count++ //Math.round(Math.random()*1000);
        ...
        ;
    if (!sess.dirName) {
        sess.dirName = crypto.createHash('md5').update(sid).digest('hex');
    }

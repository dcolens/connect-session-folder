...
FileSessionStore.prototype._removePrefix = function(files) {
    var i = files.indexOf(this.prefix);
...
FileSessionStore.prototype.get = function(sid, fn) {
    ...
    fs.exists( filePath, function (exists) {
        if( exists ){
            fs.readFile( filePath, function (err, data) {
                if(err) {
                    if( printDebug ) { util.error(serial + ' get ERROR [' + filePath + '] - ', err); }
                    fn(err);                    
...
FileSessionStore.prototype.set = function (sid, sess, fn) {
    ...
    if( printDebug ) { util.log(serial + ' set [' + filePath + '] = ' + content); }
    ...
    if( !this.useAsync ){
        if (!fs.existsSync(dirPath) && !fs.mkdirSync(dirPath)) {
            ...
            if (printDebug ) {
                util.error(serial + ' set err [' + filePath + '] = ' + content);
            }
        } else if( printDebug ) {
            util.log(serial + ' set sync OK [' + filePath + '] = ' + content);
        }
            ...
            if (!exists) {
                fs.mkdir(dirPath, function(err) {
                    ...
                    fs.writeFile( filePath, content, function(err) {
                        ...
                        if( printDebug ) { util.log(serial + ' set OK [' + filePath + '] = ' + content); }
                        fn && fn();
                    });
                });
            } else {

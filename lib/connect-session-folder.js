...
module.exports = function(connect){
	...
	function SessionFolderMemory(options) {
		...
		qfs.exists(this.parentFolder).then(function(exists) {
			...
			setTimeout(function() { 
				/*this is an ugly hack to override the sessionid generator, hopefully 
				https://github.com/senchalabs/connect/issues/641 gets a solution and we 
				can do this properly.
				*/
			  self.generate = function(req){
			    req.sessionID = req.user;
			    req.session = new connect.session.Session(req);
			    req.session.cookie = new connect.session.Cookie(connect.session.cookie);
			  };
			  self.emit('connect');
			}, 100);
		}, function(e) {
